...
В качестве параметров задается:
  кол-во этажей в подъезде — N (от 5 до 20)
  высота одного этажа
  скорость лифта при движении в метрах в секунду
  (ускорением пренебрегаем, считаем, что когда лифт едет — он сразу едет с определенной скоростью)
  время между открытием и закрытием дверей

После запуска программа должна постоянно ожидать ввода от пользователя
и выводить действия лифта в реальном времени.
